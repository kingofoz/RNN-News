...
class TweetCollector(object):
    def __init__(self, agencies, token, token_secret, consumer, consumer_secret):
        ...
        self._refresh = 10 * 60
        self._tweets = TinyDB('./data/resources/tweets.json')
        self._tweets.purge()
    ...
    def _single_execute(self):
        ...
        for agency in self.agencies:
            tweeter_handle = self.agencies[agency]['handle']
            status = self._status(tweeter_handle)
            if status:
                if not self._tweets.search(where('id') == status._json['id']) or not self._tweets.all():
                    self._tweets.insert(status._json)
            else:
                continue
